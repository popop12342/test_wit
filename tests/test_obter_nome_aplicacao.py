...
class TestObterNomeAplicacao(unittest.TestCase):
    ...
    def test_nome_aplicacao_inicialmente_fornecido(self):
        resposta = {'_text': ['aplicar', 'viagem', 'mesmo valor'], 'entities': {'intent': [{'confidence': 0.98450955141391, 'value': 'aplicar'}], 'nome_aplicacao': [{'confidence': 0.94776218843284, 'value': 'viagem', 'type': 'value'}], 'amount_of_money': [{'confidence': 0.9635075, 'value': 100, 'type': 'value', 'unit': 'BRL'}]}, 'msg_id': '0cPX2dM45YObKjVGB'}
        nome_aplicacao = obter_nome_aplicacao(io=self.io, resposta=resposta, cliente=self.cliente, modo=self.modo)
        self.assertEqual(nome_aplicacao, 'viagem', "não detecta nome_aplicação")

    def test_nome_aplicacao_inicialmente_nao_fornecido(self):
        resposta = {'_text': ['aplicar'], 'entities': {'intent': [{'confidence': 0.98450955141391, 'value': 'aplicar'}]}, 'msg_id': '0cPX2dM45YObKjVGB'}
